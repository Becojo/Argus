...
@click.option(
    "--mode",
    type=click.Choice(["repo", "action"]),
    required=True,
    help="The mode of operation. Choose either 'repo' or 'action'.",
)
@click.option(
    "--url",
    required=True,
    type=str,
    help="The GitHub URL. use USERNAME:TOKEN@URL for private repos.",
)
@click.option(
    "--output-folder",
    required=False,
...
@click.option(
    "--action-path", default=None, type=str, help="The (relative) path to the action."
)
@click.option(
    "--workflow-path",
    default=None,
    type=str,
    help="The (relative) path to the workflow.",
)
def main(
    mode,
    url,
    ...
    if num_of_options_provided > 1:
        raise click.BadParameter(
            "You must provide exactly one of: --branch, --commit, --tag"
        )

    option_provided, option_value = next(
        (
            (name, value)
            for name, value in zip(options_names, options)
    ...
    option_dict = (
        {"type": option_provided, "value": option_value}
        if option_provided and option_value
        else {}
    )
    ...
    if mode == "repo":
        ...
        if workflow_path:
            raise click.BadParameter(
                "You cannot provide --workflow-path in action mode."
            )
